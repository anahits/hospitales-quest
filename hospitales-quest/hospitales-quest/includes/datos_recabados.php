	...
	<body>
		...
		<main class="container ml-2" id="main-content">			
			...
			<div class="mb-3 mt-3 ml-4">
				...
				<div class="row ml-0">Estado de residencia:<strong class="mr-2 ml-3"><?php isset($estado) ? print $estado: print "" ; ?></strong></div>
			</div>
			<hr>
			<div class="mb-3 mt-3 ml-4">
			...
			<div class="mb-3 mt-3 ml-4">
				...
				<div class="mb-3 mt-3 ml-3">
					...
					<div class="row ml-0">Días asistencia urgencias:<strong class="mr-2 ml-3"><?php  isset($numMismaConsulta['Urgencias']) ? print $diasUrgenciasTotal : print "";  ?></strong></div>
			...
			<div class="mb-3 mt-3 ml-4">
				...
				<div class="row ml-0">
					...
					<table class="table estudios mt-2 mb-2">
						<tr>
							<th class="col-sm-3">Tipo de Consulta</th>
							<th class="col-sm-7">Nombre Estudio</th>
							<th class="col-sm-2">Núm de estudios</th>
						</tr>
						<tbody>
							<?php
								foreach( $estudiosLab as $index => $estudioLab ) {
									print '<tr><td>'.$tipoConsulEstLab[$index] . '</td><td>'.$estudioLab.'</td><td>'.$cantidadEstLab[$index] . '</td></tr>';
								}	
							?>
						</tbody>
				...
				Pruebas de Alergia:
				<div class="row ml-0">
					<?php if($pruebsAlerg !== null){ ?>
					<table class="table estudios mt-2 mb-2">
						<tr>
							<th class="col-sm-3">Tipo de Consulta</th>
							<th class="col-sm-7">Nombre Prueba de alergia</th>
							<th class="col-sm-2">Núm de Prueba de alergia</th>							
						</tr>
						<tbody><?php
								foreach( $pruebsAlerg as $index => $pruebAlerg ) {
									print '<tr><td>'.$tipoConsulPruebAlerg[$index] . '</td><td>'.$pruebAlerg.'</td><td>'.$cantidadpruebAlerg[$index] . '</td></tr>';
							}?>
						</tbody>
					</table><?php } else{ echo '<strong class="mr-2 ml-3">Ninguno</strong>';}?>
				</div>
					...
					<table class="table estudios mt-2 mb-2">
						...
						<tbody><?php
								foreach( $estudiosGab as $index => $estudioGab ) {
									print '<tr><td>'.$tipoConsulEstGab[$index] . '</td><td>'.$estudioGab.'</td><td>'.$cantidadEstGab[$index] . '</td></tr>';
							}?>
						</tbody>
				...
				<div class="row ml-0">
					...
					<table class="table estudios mt-2 mb-2">
						<tr>
							<th class="col-sm-3">Tipo de Consulta</th>
							<th class="col-sm-7">Nombre Porcedimiento</th>
							<th class="col-sm-2">Núm de Porcedimiento</th>
							
						</tr>
						<tbody><?php
								foreach( $procedimientos as $index => $procedimiento ) {
									print '<tr><td>'.$tipoConsulProced[$index] . '</td><td>'.$procedimiento.'</td><td>'.$cantidadProced[$index] . '</td></tr>';
							}?>
						</tbody>
					</table><?php } else{ echo '<strong class="mr-2 ml-3">Ninguno</strong>';}?>
				</div>
